...
export async function handleExecutiveValueReport(format: "pdf" | "xlsx") {
  ...
  const [renewalRows, findingsResult, connectionsResult, confirmedOutcomes] = await Promise.all([
    getExportRows(context.organizationId, "basic_contract_register"),
    supabase.from("license_waste_opportunities")
      .select("id, finding_type, review_status, estimated_savings, realized_savings, currency, confidence, is_sample, resolved_at, superseded_at, matched_contract_ids, involved_providers")
  ...
  const input: ExecutiveValueReportInput = {
    ...
    protectedDeadlineCount: renewalRows.filter((row) => Boolean(row.notice_deadline_date) && String(row.needs_review ?? "").toLowerCase() !== "yes").length,
    providerFreshness: (connectionsResult.data ?? []).map((row) => ({ provider: row.provider, lastSuccessfulSyncAt: row.last_successful_sync_at })),
  ...
  const buffer = format === "pdf" ? buildExecutiveValuePdf(input) : buildExecutiveValueWorkbook(input);
