  ...
  return {
    ...
    recommendedActions: [
      ...(input.ruleOutcomes ?? [])
        ...
        .map((outcome) => ({ code: outcome.code, label: outcome.recommendedAction ?? outcome.message, severity: outcome.severity })),
      ...openDecisions
        .filter((decision) => decision.decisionType === "recommendation" || decision.decisionType === "next_action")
        .map((decision) => ({ code: decision.ruleId ?? decision.id, label: decision.title, severity: decision.severity }))
    ],
    blockers: [
      ...(input.ruleOutcomes ?? [])
        .filter((outcome) => outcome.outcomeType === "blocker")
