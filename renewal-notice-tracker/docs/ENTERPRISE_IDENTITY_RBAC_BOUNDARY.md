...
The current runtime bridge for future implementation lives in `lib/product/enterprise-identity-runtime.ts`. It centralizes Enterprise-plan admin/owner gating, SSO configuration readiness shaping, prepared SAML/OIDC configuration contracts, SSO callback decision shaping from verified provider results, hashed-identifier SSO callback decisions, SCIM bearer-token authentication contracts, SCIM endpoint response/mutation decision shaping, SCIM lifecycle normalization, deprovisioned/locked-user access blocking, group-role mapping safety, break-glass admin preservation, session revocation intent/status decisions, and safe audit metadata shaping without exposing live SSO or SCIM routes.
...
| --- | --- | --- | --- | --- |
| `admin` | shipped | customer app | Upload/import, P0 review, owner assignment, reminders, decisions, exports, financial/procurement intelligence, billing/settings | Owner-only workspace deletion, internal rescue, live SSO/SCIM until the Enterprise gate ships |
...
| `owner` | shipped | customer app | Acknowledge, decide, close/reopen, basic export, owner-scoped risk access, billing/settings, deletion request | Review/trust edits, owner assignment, rich sensitive exports, internal rescue, live SSO/SCIM until the Enterprise gate ships |
...
- Future Enterprise identity route/service work must use `lib/product/enterprise-identity-runtime.ts` for Enterprise admin/owner gating, provisioning state normalization, group-role mapping safety, and audit metadata sanitization before touching session, membership, or audit paths.
- Provider-driven role/group mapping must never grant `owner`, internal roles, or future enterprise roles. `admin` group mapping is denied by default and can be enabled only by an explicit future Enterprise policy flag with audit evidence. Privileged recovery and owner assignment require explicit in-app authority and audit evidence.
...
- Prepared route-adjacent contracts for SSO callback verification results, SCIM bearer-token authentication, SCIM provisioning response/error shapes, persistence adapters, and session revocation adapters.
